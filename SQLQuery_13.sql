...
SELECT car.marca, car.modelo, SUM(via.distanciaPercorrida) AS distancia_percorrida, emp.nome
FROM carro car, viagem via, empresa emp
WHERE via.idCarro = car.id AND car.idEmpresa = emp.id
...
SELECT car.marca, car.modelo, COUNT(via.data) AS quantidade
FROM carro car, viagem via, empresa emp
WHERE via.idCarro = car.id AND car.idEmpresa = emp.id
GROUP BY car.id, car.marca, car.modelo, emp.nome
ORDER BY emp.nome ASC, quantidade DESC
...
SELECT emp.nome, car.marca, car.modelo, via.distanciaPercorrida,
	CASE
		WHEN via.distanciaPercorrida <= 1000 THEN
			10
		ELSE
			((via.distanciaPercorrida / 1000) *15)
	END AS valor_total_ganho_por_viagem
FROM carro car, viagem via, empresa emp
WHERE via.idCarro = car.id AND car.idEmpresa = emp.id
GROUP BY emp.nome, car.marca, car.modelo, via.distanciaPercorrida