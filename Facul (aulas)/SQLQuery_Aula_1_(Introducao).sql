...
que come�a em 4001 e incrementa de 1 em 1, o nome do time que tamb�m 
deve ser �nico e a cidade. Um time tem muitos jogadores, por�m 1 jogador 
s� pode jogar em 1 time e os jogadores s�o cadastrados por um c�digo 
�nico que inicia em 900101, incrementa de 1 em 1, o nome do jogador 
deve ser �nico tamb�m, sexo pode apenas ser M ou F, mas a maioria dos 
jogadores � Homem, altura com 2 casas decimais e a data de nascimento 
(apenas jogadores nascidos at� 31/12/1999). 
Jogadores do sexo masculino devem ter, no m�nimo 1.70 de altura e do 
sexo feminino, 1.60).
-------------------------------------------------------------------
A seguir se previu a necessidade de cadastro para gin�sio, por um id 
num�rico e nome char(10). Ainda n�o estava certo se id seria a chave 
prim�ria, portanto � criada sem. 
Todo gin�sio tem um time e todo time tem apenas um gin�sio. Essa 
informa��o foi passada ap�s a cria��o da tabela, al�m da confirma��o
de id como PK.
O nome do gin�sio, deve ser modificado para VARCHAR(50)
Todo gin�sio passou a necessitar armazenar uma URL com a foto da 
fachada, chamada URL.
Na sequ�ncia se verificou que esse atributo deveria se chamar 
URLFoto.
Posteriormente se verificou que esse atributo se tornou desnecess�rio e 
deve ser excluido
*/
...
PK - Cria um �ndice para busca e define unicidade
FK - Cria uma refer�ncia entre PK e FK para fazer valida��es (N�O importa dados de outra tabela)
IDENTITY(X,Y) - Auto-incremento, inicia em X, Y define o passo 
DEFAULT(X) - Caso a coluna n�o receba valor, ser� preenchida com X
UNIQUE - Define unicidade
CHECK - Valida alguma regra, caso falhe, o registro � exclu�do (N�vel de coluna ou n�vel de tabela)
*/
...
GO
--UPPER(char): Aumenta a express�o para mai�sculo
...
Se est� na ordem de cria��o das colunas
...
(1, 'Gin�sio do Aeroporto', 4002)
 
INSERT INTO ginasio VALUES
(2, 'Gin�sio Praiano', 4005)
...
DELETE times
WHERE cidade = 'Tatu�' OR cidade = 'S�o Roque' OR nome = 'Bulls'
...
--Ap�s deletar a tabela times e a recriar manualmente:
...
/*
DELETE COMPLETO (Todas as linhas, N�o funciona caso a tabela tenha FKs)
...
UPDATE times
SET nome = 'Blackers', cidade = 'Ribeir�o Preto'
WHERE id = 4008
-- OU
UPDATE times
SET nome = 'Blackers', cidade = 'Ribeir�o Preto'
...
--ERRADO, ir� atualizar todas as colunas da tabela
UPDATE times 
SET cidade = ' S�o Paulo'
--------------------------------------------------

--Usando a segunda tabela (revis�o do conte�do de cima)
...
/*
A tabela funcion�rio tem as seguintes restri��es:
- PK - id
- id - auto incremento de 1 em 1
- n�mero - deve ser positivo
...
('Fulano',	'da Silva',	'R. Volunt�rios da Patria',	8150,	'Santana',	'05423110',	'76895248',	'1974-05-15',	4350.00),
...
('Tirano',	'De Souza',	'Avenida �guia de Haia', 4430,	'Artur Alvim',	'05448000',	NULL,	NULL,	'1975-10-15',	2804.00)
...
SELECT GETDATE() AS hoje
	--CONVERS�ES DE TIPO (CAST, CONVERT)
SELECT 12 AS doze
SELECT 'A12' AS adoze
...
--Id e Nome Concatenado de quem n�o tem telefone
--nome_completo
...
--Nome Concatenado e telefone (sem ddd) de quem tem telefone em ordem alfab�tica

--Nome completo, Endereco completo (Rua, n� e CEP), ddd e telefone, ordem alfab�tica crescente
SELECT id, 
...
--Nome completo, Endereco completo (Rua, n� e CEP), data_nasc (BR), ordem alfab�tica decrescente
SELECT id, 
		nome + ' ' + sobrenome AS nome_completo,
		logradouro + ',' + CAST(numero AS VARCHAR(5)) + ' - ' + cep AS endereco_completo,
...
('Fulano','da Silva Jr.','R. Volunt�rios da Patria',8150,NULL,'05423110','11','32549874','1990-09-09',1235.00),
('Jo�o','dos Santos','R. Anhaia',150,NULL,'03425000','11','45879852','1973-08-19',2352.00),
...
--SUBSTRING(char, posi��o inicial, qtd de caracteres) - retorna char
...
--TRIM
--LTRIM(char) - retorna char sem espa�os � esquerda
--RTRIM(char) - retorna char sem espa�os � direta
...
--Consultar nome completo, com endere�o completo 
SELECT nome+' '+sobrenome AS nome_completo,
	CASE 
...
--Consultar nome completo, endere�o completo, cep mascarado, 
--telefone com ddd mascarado e valida��o de celular
SELECT id,
	nome+' '+sobrenome AS nome_completo,
	CASE 
		WHEN bairro IS NULL THEN
...
--Nomes completos dos Funcion�rios que N�O est�o no
--projeto Modifica��o do M�dulo de Cadastro
SELECT nome + ' ' + sobrenome AS nome_completo
FROM funcionario
WHERE id NOT IN
