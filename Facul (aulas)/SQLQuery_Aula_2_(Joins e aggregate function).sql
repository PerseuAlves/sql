...
/*
	...
	OUTER JOIN:
	SE desejados dados da tab1 que n�o tem refer�ncia em tab2
	...
	SE desejados dados da tab2 que n�o tem refer�ncia em tab1
	SELECT tab1.col1, tab2.col1, ... 
...
--Criar listas de chamadas (RA tem um (-) antes do �ltimo digito), 
--ordenados pelo nome, caso o nome tenha mais de 30 caract.
--mostrar 29 e um ponto(.) no final RA do aluno, nome do aluno, nome da mat�ria
--SQL2:
SELECT al.ra, 
...
ORDER BY al.nome
--CTRL + L mostra o plano de execu��o
...
--alunos que n�o est�o matriculados em nenhuma mat�ria
SELECT SUBSTRING(al.ra, 1, 9)+'-'+SUBSTRING(al.ra, 10, 1) AS ra,
	al.nome
...
--Notas abaixo da m�dia(6.0) em P1 ou P2, ordenados por mat�ria 
--e nome do aluno
--SQL3
SELECT SUBSTRING(al.ra, 1, 9)+'-'+SUBSTRING(al.ra, 10, 1) AS ra,
